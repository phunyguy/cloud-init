...
def create_disk():
    ...
    os.truncate(DISK_PATH, 1<<21)

#   # DOES THIS WORK !??!?!?!?!?!?!??!?!??!?!
#    subp('parted --script device {}         \
#            mklabel gpt                     \
#            mkpart primary 1MiB 1GiB'       \
#            .format(DISK_PATH).split())
    print(subp('parted --script device {} print'.format(DISK_PATH).split())[0])
    yield
    os.remove(DISK_PATH)
